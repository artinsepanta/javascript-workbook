...
const numberArray = stringsToNumbs(strNums);
 const sumEvens = numberArray.filter((item) => {
  return item % 2 === 0
...
// Find the index of the first value when added to it's index = 512 (#ATX!!)
  const atxIdx =numberArray.findIndex((item, index) => {
  return (item + index) === 512;
});
  console.log(`index: ${atxIdx}, value: ${numberArray[atxIdx]}`);
...
// const myArray = ['Light Cloud', 'Heavy Cloud', 'Showers']
const myArray = weather.map((item) => {
  return item['weather_state_name']
});
console.log('myArray', myArray)
const weatherStates =  myArray.filter(function(item, index, self) {
